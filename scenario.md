...
> **Sping 기반의 간단한 웹 어플리케이션 서버**를 예시로, 실무 관점에 맞춰서 단계별로 이미지를 개선해 나간다.
...
- 빌드를 로컬 환경에 의존하면 다른 환경에서 실행 시 문제 발생 가능
...
- 빌드 스테이지(`gradle:8.7-jdk17`)에서 소스를 빌드해서 `.jar` 생성
- 실행 스테이지에는 `eclipse-temurin:17-jre`(JRE만 포함)만 사용하여 JDK 제거로 이미지 크기 감소
...
|------|-----------|:-----------:|:---------:|:------:|------|
| Step 1 | 초기 버전 (JDK + 로컬 빌드) | 674MB | ❌ | ✅ | 최악의 상태 |
| Step 2 | 멀티 스테이지 빌드 (JDK→JRE) | 411MB | 보통 | ✅ | 핵심 최적화 |
| Step 3 | Alpine 미적용, JRE(Debian) 유지 | 411MB | 보통 | ✅ | 실무 절충안 |
| Step 4 | RUN `&&` 레이어 통합 + 캐시 제거 | 411MB | 보통 | ✅ | 레이어 정리 |
| Step 5 | 의존성/소스 레이어 분리 | 411MB | ✅ | ✅ | CI 속도 향상 |
